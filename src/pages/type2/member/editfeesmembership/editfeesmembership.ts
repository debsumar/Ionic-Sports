...
import { IonicPage, NavController, NavParams, AlertController, ToastController, ActionSheetController, Platform, Events } from 'ionic-angular';
...
export class EditFeesMembershipPage {
    ...
    applyTheme() {
        const el = document.querySelector("page-editfeesmembership");
        if (el) {
            this.isDarkTheme ? el.classList.remove("light-theme") : el.classList.add("light-theme");
        } else {
            setTimeout(() => {
                const retry = document.querySelector("page-editfeesmembership");
