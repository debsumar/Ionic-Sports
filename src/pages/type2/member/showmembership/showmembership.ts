...
import { IonicPage, NavController, NavParams, AlertController, LoadingController, ToastController, ActionSheetController, Platform, Events } from 'ionic-angular';
...
export class ShowmembershipPage {
    ...
    getUserMemberships() {
        ...
        this.httpService.post(`${API.MEMBERSHIP_USER}`, this.inputObj).subscribe((res: any) => {
            this.userMemberShip = res.data.memberships;
    ...
    //theming (mirrors member.ts): dark by default, toggle .light-theme on host
    loadTheme() {
    ...
    applyTheme() {
        const el = document.querySelector("page-showmembership");
        if (el) {
            this.isDarkTheme ? el.classList.remove("light-theme") : el.classList.add("light-theme");
        } else {
            setTimeout(() => {
                const retry = document.querySelector("page-showmembership");
                if (retry) {
                    this.isDarkTheme ? retry.classList.remove("light-theme") : retry.classList.add("light-theme");
                }
            }, 100);
        }
    }
