...
import { IonicPage, NavController, NavParams, AlertController, ActionSheetController, ToastController, Platform, Slides, Content, Events } from 'ionic-angular';
...
export class AssignmembershipsPage {
    ...
    applyTheme() {
        const el = document.querySelector("page-assignmemberships");
        if (el) {
            this.isDarkTheme ? el.classList.remove("light-theme") : el.classList.add("light-theme");
        } else {
            setTimeout(() => {
                const retry = document.querySelector("page-assignmemberships");
