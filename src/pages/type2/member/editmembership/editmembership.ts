...
import { ActionSheetController, AlertController, Checkbox, IonicPage, NavController, NavParams, PopoverController, Events } from 'ionic-angular';
...
export class EditmembershipPage {
  ...
  loadTheme() {
    this.storage.get("dashboardTheme").then((isDark) => {
      this.isDarkTheme = (isDark !== null && isDark !== undefined) ? isDark : true;
      this.applyTheme();
  ...
  applyTheme() {
    const el = document.querySelector("page-editmembership");
    if (el) {
      this.isDarkTheme ? el.classList.remove("light-theme") : el.classList.add("light-theme");
    } else {
      setTimeout(() => {
        const retry = document.querySelector("page-editmembership");
        if (retry) { this.isDarkTheme ? retry.classList.remove("light-theme") : retry.classList.add("light-theme"); }
      }, 100);
    }
  }


  //row click toggles selection (whole row is tappable)
  toggleMember(member: FamilyMemberModel) {
    if (member.member_enrolled) {
      return;
    }
    const willSelect = !member.IsSelect;
    if (willSelect) {
