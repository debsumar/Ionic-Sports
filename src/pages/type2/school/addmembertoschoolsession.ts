...
import { LoadingController, NavController, NavParams, ViewController, AlertController, Checkbox, Events } from 'ionic-angular';
...
export class Type2AddMemberSchoolSession {
  ...
  private onThemeChanged = (isDark: boolean) => {
  ...
  constructor(public commonService: CommonService, private alertCtrl: AlertController, public storage: Storage,
    ...
    public popoverCtrl: PopoverController, private themeService: ThemeService,
  ...
  ionViewWillEnter() {
    ...
    this.events.subscribe('theme:changed', this.onThemeChanged);
  }

  ionViewWillLeave() {
    this.events.unsubscribe('theme:changed', this.onThemeChanged);
  ...
  private loadTheme(): void {
    this.storage.get('dashboardTheme').then((isDarkTheme) => {
      this.isDarkTheme = isDarkTheme !== null && isDarkTheme !== undefined ? isDarkTheme : true;
    }).catch(() => {
  ...
  applyTheme() {
    const pageElement = document.querySelector('addmembertoschoolsession-page');
    if (pageElement) {
