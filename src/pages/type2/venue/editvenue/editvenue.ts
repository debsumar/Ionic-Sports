...
import { NavController, PopoverController, NavParams, ToastController, Events } from 'ionic-angular';
...
export class Type2EditVenue {
    ...
    constructor(public storage: Storage, public comonService: CommonService,public toastCtrl: ToastController, public navParams: NavParams, public navCtrl: NavController,
      ...
      this.clubId = this.navParams.get("venue").Id;
      this.getVenue();
  ...
  applyTheme(isDark: boolean) {
    this.isDarkTheme = isDark;
    const pageElement = document.querySelector('editvenue-page');
    if (pageElement) {
  ...
  getVenue = () => {
        ...
        const params = new HttpParams().set('clubId', this.clubId);
        this.httpService.get(API.GET_CLUB, params, null, 1).subscribe((res: any) => {
            this.comonService.hideLoader();
            const club_array = (res && res.data) ? res.data : [];
            this.clubDets = club_array.length > 0 ? club_array[0] : ({} as Venue);
            this.isVenue_available = club_array.length > 0;
        }, (err) => {
            this.comonService.hideLoader();
    ...
    //   }
      ...
      updatePostgreVenue() {
        ...
        this.httpService.put(API.UPDATE_VENUE, venue_updateInput, null, 1).subscribe(
          (res: any) => {
            this.comonService.hideLoader();
            console.log("venue data" + JSON.stringify(res && res.data));
