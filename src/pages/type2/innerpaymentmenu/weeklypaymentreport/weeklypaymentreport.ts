...
export class WeeklyPaymentReport {
  ...
  paidMemberList: Array<WeeklyPaymentReportRes> = [];
  ...
  getPayment(): void {
    this.httpService.post<{ message: string, data: WeeklyPaymentReportRes[] }>(`${API.WEEKLY_ALL_PAYMENT_DETAILS}`, this.report_input)
      .subscribe({
        next: (res) => {
  ...
  getFilterItems(ev: any) {
    ...
    if (val && val.trim() != '') {
      this.paidMemberListtemp = this.paidMemberList.filter((item) => {
        if (item.member_name != undefined && item.member_name.toLowerCase().indexOf(val.toLowerCase()) > -1) {
          return true;
        }
        if (item.session_name != undefined && item.session_name.toLowerCase().indexOf(val.toLowerCase()) > -1) {
          return true;
        }
        if (item.coach_name != undefined && item.coach_name.toLowerCase().indexOf(val.toLowerCase()) > -1) {
          return true;
        }
        if (item.club_name != undefined && item.club_name.toLowerCase().indexOf(val.toLowerCase()) > -1) {
          return true;
        }
        if (item.days != undefined && item.days.toLowerCase().indexOf(val.toLowerCase()) > -1) {
          return true;
        }
      });
    } else {
