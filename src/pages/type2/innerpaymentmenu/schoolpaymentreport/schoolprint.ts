...
@IonicPage()
@Component({
  selector: 'page-schoolprint',
  templateUrl: 'schoolprint.html',
  providers: [HttpService]
})
export class SchoolReportPrint {
  ...
  getParentClubInfo() {
    let parentclubObs = this.fb.getAllWithQuery("ParentClub/Type2", { orderByKey: true, equalTo: this.parentClubKey }).subscribe((data) => {
      ...
      this.selectOBj.attachmentName = `School_Payment_Report@${this.selectOBj.parentClubName+ new Date().getTime()}`;
  ...
  preparememberObj() {
    ...
    this.memberList.forEach((member) => {
      ...
      if (member.SchoolName != undefined) {
        tempMemberObj.SchoolName = member.SchoolName;
      } else {
        tempMemberObj.SchoolName = "Not Available";
      }
    ...
    return new Promise((res,rej)=>{
      this.selectOBj.memerList.forEach((mem)=>{
        ...
        if(!this.selectOBj.isSession){
          delete mem.SessionName;
        }
        if(!this.selectOBj.isVanue){
          delete mem.SchoolName;
        }
      })
      this.selectOBj.ccAddress = this.parentClubInfo.ParentClubAdminEmailID;
      this.selectOBj.subject = `SchoolSession Report @${this.selectOBj.parentClubName}`;
  ...
  sendXLS(){
    return new Promise((res,rej)=>{
     ...
     try{
      let sentObj = {
          toAddress: this.selectOBj.toAddress,//need to remove this.selectOBj.toAddress,
          ...
          subject:`SchoolSession Report@${this.selectOBj.parentClubName}`,
      ...
      this.httpService.post(`${API.ATTACHMENT_EMAIL}`, sentObj).subscribe((response: any) => {
        res(response);
      }, (error) => {
        rej(error);
      });
