...
export class ShowvideofromlistPage {
  ...
  constructor(public socialSharing: SocialSharing, public fb: FirebaseService, private dom: DomSanitizer, public navCtrl: NavController, public navParams: NavParams, public events: Events, public storage: Storage, private themeService: ThemeService) {
  ...
  applyTheme(isDark: boolean) {
    const element = document.querySelector('page-showvideofromlist');
    if (element) {
      if (isDark) {
      ...
      setTimeout(() => {
        const retryElement = document.querySelector('page-showvideofromlist');
        if (retryElement) {
          if (isDark) {
