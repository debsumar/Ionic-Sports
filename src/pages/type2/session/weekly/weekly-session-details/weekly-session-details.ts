...
import {
  WeeklySessionDateDetailsInput,
  WeeklySessionMember,
} from "../weeklydatedetails.model";
...
import {
  AppType,
  ModuleTypes,
} from "../../../../../shared/constants/module.constants";
...
export class WeeklySessionDetailsPage {
  ...
  applyTheme() {
    const pageElement = document.querySelector("page-weekly-session-details");
    if (pageElement) {
  ...
  async WeeklyPrivacyConfirmation(Session: WeeklySessionDays) {
    ...
    const confirmationMessage =
      Session.private_status === 1
        ? `Do you want to make "${sessionName}" private?`
        : `Do you want to make "${sessionName}" public?`;
  ...
  hideSession(sesData: WeeklySessionDays, newPrivacyStatus: number) {
    try {
      ...
      const cancel_weekly_sess = gql`
        mutation toggleWeeklySessionDatePrivate(
          $input: ToggleSessionDatePrivateInput!
        ) {
      ...
      const cancel_weekly_variable = {
        input: this.toggleSessionDatePrivateInput,
      };

      this.graphqlService
        .mutate(cancel_weekly_sess, cancel_weekly_variable, 0)
        .subscribe(
          (response) => {
  ...
  ToggleSessions() {
    ...
    this.weeklySessionDetailsInput.fetchActiveSession =
      this.isShowFutureSnsOnly;
    this.weeklySessionDetails();
  }

  async notifySessionUsers(sesData: WeeklySessionDays) {
    try {
      const member_data = (await this.getSessionDets(
        sesData,
      )) as WeeklySessionMember[];
      if (member_data.length > 0) {
        const member_ids = member_data.map((enrol_member) =>
          enrol_member.member.IsChild
            ? enrol_member.member.ParentId
            : enrol_member.member.Id,
        );
        ...
        this.commonService.toastMessage(
          "No member(s) found for the current session",
          2500,
          ToastMessageType.Error,
        );
      }
  ...
  async sendWeeklyMail(sesData: WeeklySessionDays) {
    try {
      this.membersData = (await this.getSessionDets(
        sesData,
      )) as WeeklySessionMember[];
      if (this.membersData.length > 0) {
        const member_list = this.membersData.map(
          (enrol_member: WeeklySessionMember, index) => {
            return {
              IsChild: enrol_member.member.IsChild ? true : false,
              ParentId: enrol_member.member.IsChild
                ? enrol_member.member.ParentId
                : "",
              MemberId: enrol_member.member.Id,
              MemberEmail:
                enrol_member.member.EmailID != "" &&
                enrol_member.member.EmailID != "-" &&
                enrol_member.member.EmailID != "n/a"
                  ? enrol_member.member.EmailID
                  : enrol_member.member.IsChild
                    ? enrol_member.member.ParentEmailID
                    : "",
              MemberName:
                enrol_member.member.FirstName +
                " " +
                enrol_member.member.LastName,
            };
          },
        );
        const session = {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const weeklySessionDateDetails = gql`
          query getWeeklySessionDateDetails(
            $weekly_dets_input: WeeklySessionDateDetailsInput!
          ) {
        ...
        this.graphqlService
          .query(
            ...
            { weekly_dets_input: { SessionDateId: sesData.id } },
            0,
          )
          .subscribe(
            (res: any) => {
              this.commonService.hideLoader();
              resolve(
                res.data.getWeeklySessionDateDetails
                  .weeklySessionMember as WeeklySessionMember[],
              );
            },
            (error) => {
              this.commonService.hideLoader();
  ...
  weeklySessionDetails() {
    ...
    `;
    this.graphqlService
      .query(weeklySessionDetails, { input: this.weeklySessionDetailsInput }, 0)
      .subscribe(
        (data: any) => {
          //   this.commonService.hideLoader();
          ...
          const allSessions =
            data.data.getWeeklySessionDetails.weeklySessionDays;
          if (this.isShowFutureSnsOnly) {
            // Filter future sessions
            this.individualDetails = allSessions.filter((session) =>
              moment(session.session_date).isSameOrAfter(moment(), "day"),
            );
          } else {
            // Filter past sessions
            this.individualDetails = allSessions.filter((session) =>
              moment(session.session_date).isBefore(moment(), "day"),
            );
          }
          // console.log("Weekly Details Data Is:", JSON.stringify(this.weeklyDets));
          console.log(
            "Individual Weekly Data  Is:",
            JSON.stringify(this.individualDetails),
          );
        },
        (error) => {
          //   this.commonService.hideLoader();
  ...
  addMemberToWeeklySession(sesData: WeeklySessionDays) {
    if (Number(sesData.capacity_left) > 0) {
      this.navCtrl.push("AddmembertoweeklyPage", {
        weeklySessionId: this.weeklyDets.id,
        individualWeeklyId: sesData.id,
      });
    } else {
      this.commonService.toastMessage(
        "Session is full",
  ...
  removeWeeklySession() {
    ...
    this.commonService.commonAlert_V3(
      "Delete Session",
      ...
      () => {
        try {
          ...
          this.graphqlService
            .mutate(delete_weekly_sess, delete_weekly_variable, 0)
            .subscribe(
              (response) => {
  ...
  sendEmailsAfterWeeklyCancellation(session: WeeklySessionDays) {
    ...
    try {
      let notificationDetailsObjForMember = {
        ...
        ClubShortName:
          this.weeklyDets.club.ClubName == undefined
            ? ""
            : this.weeklyDets.club.ClubName,
      ...
      for (
        let memberIndex = 0;
        memberIndex < this.membersData.length;
        memberIndex++
      ) {
        if (this.membersData[memberIndex].member.EmailID.trim() != "") {
          ...
          if (memberEmail != "") {
            emailFormembers.Members.push({
              ...
              MemberName:
                this.membersData[memberIndex].member.FirstName +
                " " +
                this.membersData[memberIndex].member.LastName,
            });
            notificationDetailsObjForAdmin.Member[mKey] = {
              MemberKey: mKey,
              MemberName:
  ...
  EditWeekly() {
    const booking_count = this.weeklyDets.weeklySessionDays.filter(
      (x) => Number(x.bookingCount) > 0,
    ).length;
    this.navCtrl.push("EditweeklysessionPage", {
      weeklysesdets: this.weeklyDets,
      is_enrolls_there: booking_count > 0 ? true : false,
    });
  }

  //this is to weekly sessiondets
  GotoSesDets(sesData: WeeklySessionDays) {
    this.navCtrl.push("WeeklysessiondetsPage", {
      weekly_session: this.weeklyDets,
      individualWeekly: sesData.id,
    });
  }
  ...
  getWaitingListData() {
    ...
    this.httpService
      .post(API.WAITING_LIST_GET_BY_MODULE, requestBody)
      .subscribe({
        next: (data: any[]) => {
          this.commonService.hideLoader();
          this.waitingListData = data.map((item) => ({
            ...
            member_name:
              item.member && item.member.FirstName && item.member.LastName
                ...
                : "Unknown Member",
            individual_session_name:
              item.session && item.session.session_name
  ...
  ShowWaitingListActionSheet(waitingItem: any) {
    ...
    if (waitingItem.status === 0) {
      ...
      buttons.push({
        ...
        icon: "ios-undo",
        handler: () => {
          this.updateWaitingListStatus(
            waitingItem,
            0,
            "Moving back to waiting list...",
          );
        },
      });
    }
  ...
  updateWaitingListStatus(
    ...
    this.httpService
      .post(API.WAITING_LIST_UPDATE_STATUS, requestBody)
      .subscribe({
        next: (data) => {
          this.commonService.hideLoader();
