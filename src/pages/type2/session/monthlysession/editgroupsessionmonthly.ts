...
import {
  ...
  MonthlySessionEdit,
} from "./model/monthly_session_create.model";
...
export class Type2EditGroupSessionMonthly {
  ...
  constructor(
    ...
    this.maxDate = moment(
      moment(this.postgre_session_input.EndDate).add(1, "year"),
    )
      .format("YYYY-MM-DD")
      .toString();
      ...
      .then(([userObj, currencyDetails, postgre_parentclub]) => {
        if (userObj) {
          ...
          if (val.$key != "") {
            ...
            this.postgre_session_input.user_postgre_metadata.UserMemberId =
              val.$key;
          }
        }
        if (currencyDetails) {
    ...
    this.selectedActivityTypeKey =
      this.session_dets.ActivityDetails.FirebaseActivityKey;
    this.selectActivityCategoryKey =
      this.session_dets.firebase_activity_categorykey;
    ...
    this.isExistActivitySubCategory =
      this.session_dets.is_exist_activity_subcategory;
    ...
    switch (parseInt(this.session_dets.free_months)) {
      ...
      case 1:
        ...
        break;
      default:
        this.freeSessionInTermsOfMonth =
          this.session_dets.free_months + " Months";
        break;
    }
        ...
        let alert = this.alertCtrl.create({
          ...
          buttons: [
            ...
            },
            {
              ...
              role: "cancel",
              handler: (data) => {
                this.postgre_session_input.IsAllowGroupChat =
                  !this.postgre_session_input.IsAllowGroupChat;
              },
            },
          ],
  ...
  getCoachListForGroup() {
    ...
    `;
    this.graphqlService
      .query(activity_coaches_query, { input_obj: club_coaches_input }, 0)
      .subscribe(
        (res: any) => {
          if (res.data.getAllActivityCoachs.length > 0) {
            ...
            if (this.coachs.length > 0) {
              this.selectedCoachKey =
                this.session_dets.coaches[0].coach_firebase_id;
  ...
  getActivityCategoryList() {
    ...
    this.graphqlService
      .query(
        ...
        { input_obj: activity_categories_input },
        0,
      )
      .subscribe(
        (res: any) => {
          this.activityCategoryList = res.data
            .getAllActivityCategories as ActivityCategory[];
  ...
  getActivitySubCategoryList() {
    ...
    const activity_sub_categories_query = gql`
      query getAllActivityCategorySubCategories(
        $input_obj: ActivityInfoInput!
      ) {
        getAllActivityCategorySubCategories(
          activity_subcategory_input: $input_obj
        ) {
    ...
    this.graphqlService
      .query(
        activity_sub_categories_query,
        { input_obj: activity_subcategories_input },
        0,
      )
      .subscribe(
        (res: any) => {
          this.activitySubCategoryList = res.data
            .getAllActivityCategorySubCategories as ActivitySubCategory[];
          if (this.activitySubCategoryList.length > 0) {
            if (!this.selectActivitySubCategoryKey) {
              //first time
              this.selectActivitySubCategoryKey =
                this.session_dets.firebase_activity_subcategorykey;
            } else {
              this.selectActivitySubCategoryKey =
                this.activitySubCategoryList[0].ActivitySubCategoryId;
            }
          } else {
            this.commonService.toastMessage(
  ...
  showEndDatePopUp() {
    ...
    const max = moment(this.postgre_session_input.EndDate, "YYYY-MM-DD")
      .add(5, "years")
      .format("DD-MM-YYYY");
      ...
      [
        {
          ...
          max: max,
          value: moment(
            this.postgre_session_input.EndDate,
            "YYYY-MM-DD",
          ).format("DD-MM-YYYY"),
        ...
        },
        {
          ...
          handler: (data) => {
            if (data.end_date) {
              ...
              if (
                this.commonService.validateStartAndEndDate(
                  this.postgre_session_input.StartDate,
                  data.end_date,
                )
              ) {
                this.postgre_session_input.NoOfWeeks =
                  this.commonService.calculateWeksBetweenDates(
                    this.postgre_session_input.StartDate,
                    this.postgre_session_input.EndDate,
                  );
  ...
  updateMonthlySessionEndDate(end_date) {
    ...
    this.httpService
      .post<{
        message: string;
        data: Boolean;
      }>(`${API.UPDATE_MONTHLY_SESSION_ENDDATE}`, stats_input)
      .subscribe({
        next: (res) => {
  ...
  selectedPayByDate(event) {
    ...
    this.selector
      .show({
        ...
        items: [this.jsonPayByDateData.month],
      })
      .then(
        (result) => {
  ...
  isAnyPlanPriceChanged(): boolean {
    for (const plan of this.session_dets.payplans) {
      ...
      const original_non_member_price = this.payplans.get(
        plan.plan_id_non_member,
      );
      if (
  ...
  async updatePostgreSession() {
    this.postgre_session_input.GroupSize = Number(
      this.postgre_session_input.GroupSize,
    );
    this.postgre_session_input.GroupStatus = Number(
      this.postgre_session_input.GroupStatus,
    );
    this.postgre_session_input.Duration =
      this.postgre_session_input.Duration.toString();
    this.postgre_session_input.NoOfWeeks = Number(
      this.postgre_session_input.NoOfWeeks,
    );
    this.postgre_session_input.category_id = this.selectActivityCategoryKey;
    this.postgre_session_input.category_name = this.activityCategoryList.filter(
      (category) =>
        category.ActivityCategoryId === this.selectActivityCategoryKey,
    )[0].ActivityCategoryName;
    this.postgre_session_input.sub_category_id =
      this.selectActivitySubCategoryKey;
    this.postgre_session_input.sub_category_name =
      this.activitySubCategoryList.filter(
        (category) =>
          category.ActivitySubCategoryId === this.selectActivitySubCategoryKey,
      )[0].ActivitySubCategoryName;
    ...
    for (const plan of this.session_dets.payplans) {
      ...
      if (
        member_plan_price != plan.plan_amount_member ||
        non_member_plan_price != plan.plan_amount_non_member
      ) {
        this.postgre_session_input.PayPlans.push({
          noOfdays: plan.days_for,
          member_price:
            member_plan_price != plan.plan_amount_member
              ? plan.plan_amount_member
              : undefined,
          non_member_price:
            non_member_plan_price != plan.plan_amount_non_member
              ? plan.plan_amount_non_member
              : undefined,
        });
      }
    }
    ...
    this.postgre_session_input.session_postgre_fields.coach_ids.push(
      await this.getCoachIdsByFirebaseKeys(this.selectedCoachKey),
    );
    ...
    const monthly_mutation_variable = {
      sessionInput: this.postgre_session_input,
    };
    this.graphqlService
      .mutate(monthly_ses_mutation, monthly_mutation_variable, 0)
      .subscribe(
        (response) => {
          const message =
            "Session updated successfully. Please add member(s) to the session.";
          this.commonService.toastMessage(
            message,
            2500,
            ToastMessageType.Success,
  ...
  getCoachIdsByFirebaseKeys(coach_ids): Promise<any> {
    return new Promise((res, rej) => {
      ...
      this.graphqlService
        .query(coach_query, coach_query_variables, 0)
        .subscribe(
          (response) => {
  ...
  validateSessionDate() {
    if (
      new Date(this.postgre_session_input.StartDate).getTime() >
      new Date(this.postgre_session_input.EndDate).getTime()
    ) {
      this.commonService.toastMessage(
        "Session start date should be greater than end date.",
        2500,
      );
      return false;
    }
  ...
  selectFreeSessions() {
    this.selector
      .show({
        ...
        theme: this.platform == "android" ? "dark" : "light",
        items: [this.jsonData.month],
      })
      .then(
        (result) => {
          this.freeSessionInTermsOfMonth = result[0].description;
          if (this.freeSessionInTermsOfMonth != "None") {
            this.sessionDetails.FreeSesionIntermsOfMonth =
              result[0].description[0] + result[0].description[1];
          } else {
  ...
  numberOfMonthMustPay() {
    this.selector
      .show({
        title: "Pay in advance",
        theme: this.platform == "android" ? "dark" : "light",
        items: [this.jsonDataInMonth.payInAdvanceInMonth],
      })
      .then(
        (result) => {
          this.payInAdvance = result[0].description;
          if (this.payInAdvance != "None") {
            this.sessionDetails.NoOfMonthMustPay =
              result[0].description[0] + result[0].description[1];
          } else {
