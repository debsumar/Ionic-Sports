...
@IonicPage()
@Component({
  selector: 'page-newcreatepriceband',
  templateUrl: 'newcreatepriceband.html',
})
export class NewcreatepricebandPage {
  ...
  courtselectionlist = ["All", "Outdoor", "Indoor", "Select-facilities"]
  constructor(private alertCtrl: AlertController, public toastCtrl: ToastController, public comonService: CommonService, public loadingCtrl: LoadingController, storage: Storage,
    ...
    if (this.PriceBand != undefined) {
      ...
      this.selectedOption = this.courtselectionlist.indexOf(this.PriceBand.CourtType)
      ...
      this.comonService.convertFbObjectToArray(this.PriceBand.Court).forEach(court => {
        if (court.IsActive) 
        this.courtArray.push(court.Key)
      })
    } else {
      this.PriceBand = {
        ActivityDuration: '', ActivityKey: '', ActivityName: '', PriceBandName: '', ClubKey: '', ClubName: '', Court: {}, CourtType: 'Outdoor', Status: true,
        Comments: '', Days: '', IsActive: 'True', IsEnable: 'True', CreatedDate: '', UpdatedDate: '', DurationForMember: '',
        CostForMemberBetweenDuration: '', CostForMemberAfterDuration: '', DurationForNonMember: '', CostForNonMemberBetweenDuration: '', CostForNonMemberAfterDuration: '', CostForGuestBetweenDuration: '',
        CostForGuestAfterDuration: ''
      }
    }
  ...
  getcourts(index) {
    switch (+index) {
      case 0:
        this.getTotalCourtSetup()
        break
      case 1:
        this.selectedcourtType = "Outdoor"
        this.getAllCourtSetup()
        break
      case 2:
        this.selectedcourtType = "Indoor"
        this.getAllCourtSetup()
        break
      case 3:
        this.getTotalCourtSetup()
  ...
  getAllCourtSetup() {
    this.fb.getAllWithQuery("/Court/" + this.parentClubKey + "/" + this.selectedClub + "/" + this.selectedActivity + "/", { orderByChild: 'CourtType', equalTo: this.selectedcourtType }).subscribe((data) => {
      this.courtSetup = [];
      if (data.length > 0) {
        //this.selectedCourt = data[0].$key
        data.forEach(court => {
          if (court.IsActive) {
            this.courtSetup.push(court)
          }
  ...
  courtChange(court) {
    if (court == "all") {
      this.courtArray = []
      this.courtSetup.forEach(setup => {
        this.courtArray.push(setup.$key)
      })
    }
  }
  ...
  savePriceBand() {
    ...
    this.PriceBand.CourtType = this.courtselectionlist[+this.selectedOption]
    ...
    if (this.PriceBand.PriceBandName != "" && this.PriceBand.CostForMemberAfterDuration != "" && this.PriceBand.CostForMemberBetweenDuration != "" && this.PriceBand.CostForNonMemberAfterDuration != "" && this.PriceBand.CostForNonMemberBetweenDuration != "") {
      let key = this.fb.save(this.PriceBand, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey);
      if (+this.selectedOption == 3) {
        this.courtArray.forEach(courtkey => {
            this.fb.update(courtkey, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
              IsActive: true,
              CreationDate: new Date().getTime()
            })
        })
      } else {
        this.courtSetup.forEach(court => {
            this.fb.update(court.$key, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
              IsActive: true,
              CreationDate: new Date().getTime()
            })
        })
  ...
  updatePriceBand() {
    ...
    if (this.PriceBand.PriceBandName != "" && this.PriceBand.CostForMemberAfterDuration != "" && this.PriceBand.CostForMemberBetweenDuration != "" && this.PriceBand.CostForNonMemberAfterDuration != "" && this.PriceBand.CostForNonMemberBetweenDuration != "") {
      ...
      // if (+this.selectedOption == 3) {
      //   let PriceBandCourt = this.comonService.convertFbObjectToArray(this.PriceBand.Court)
      //   this.courtArray.forEach(courtkey => {
      //     if (!this.PriceBand.Court) {
      //       this.fb.update(courtkey, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
      //         IsActive: true,
      //         CreationDate: new Date().getTime()
      //       })
      ...
      //       let isnewlyaddedcourt =  PriceBandCourt.some(priceCourt => priceCourt.Key == courtkey)
      //       if(!isnewlyaddedcourt){
      //         this.fb.update(courtkey, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
      //           IsActive: true,
      //           CreationDate: new Date().getTime()
      //         })
      ...
      //     }
      //   })
      //   PriceBandCourt.forEach(pricecourt =>{
      //     let courtArray = this.courtArray
      //     let isremovedcourt = courtArray.some(court => court == pricecourt.Key && court.IsActive)
      //     if(!isremovedcourt){
      //       let pricecourtKey = pricecourt.Key
      //       delete pricecourt.Key
      //       this.fb.update(pricecourtKey, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
      //         IsActive: false,
      //         CreationDate: pricecourt.CreationDate
      //       })
      //     }
      //   })
      ...
      //       })
      //     }else{           
      //       let isnewlyaddedcourt =  PriceBandCourt.some(priceCourt => priceCourt.Key == court.$key)
      //       if(!isnewlyaddedcourt){
      //         this.fb.update(court.$key, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
      //           IsActive: true,
      //           CreationDate: new Date().getTime()
      ...
      //   })
      //   PriceBandCourt.forEach(pricecourt =>{
      //     let courtSetup = this.courtSetup
      //     let isremovedcourt =  courtSetup.some(court => court.$key == pricecourt.Key && court.IsActive)
      //     if(!isremovedcourt){
      //       let pricecourtKey = pricecourt.Key
      //       delete pricecourt.Key
      //       this.fb.update(pricecourtKey, "StandardCode/PriceBand/" + this.parentClubKey + "/" + this.PriceBand.ActivityKey + "/" + key + "/Court", {
