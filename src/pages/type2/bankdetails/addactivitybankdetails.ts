...
@IonicPage()
@Component({
    selector: 'addactivitybankdetails-page',
    templateUrl: 'addactivitybankdetails.html'
})

export class Type2AddActivityBankDetails {
    themeType: number;
    ...
    bankDetailsObj = {
        ...
        SortCode: '',
        AccountNumber: '',
        InstructionForMembers: 'Please pay by BACS as per the bank details. Forward this page with details (amount, date etc.) once you have done the payment.'
    }
    bankDetailsKey: any;
    ...
    saveBankDetails() {
                    ...
                    if (this.clubs[i].Activities != undefined) {
                        for (let k = 0; k < this.clubs[i].Activities.length; k++) {
                            if (this.clubs[i].Activities[k].IsChecked == true) {
                                this.bankDetailsKey = this.fb.saveReturningKey("/BankDetails/Type2/" + this.selectedParentclubKey + "/" + this.clubs[i].$key + "/ActivityBankDetails/" + this.clubs[i].Activities[k].$key + "/", this.bankDetailsObj);
     ...
     validateBankDetails(): boolean {
        ...
        if (flag) {
            if (this.bankDetailsObj.AccountName == "" || this.bankDetailsObj.AccountName == undefined) {
                let message = "Please enter account holder name.";
