...
export class AddMorePhotosPage {
  ...
  constructor(public toastCtrl: ToastController, 
    ...
    private storage: Storage, public fb: FirebaseService,
  ...
  applyTheme(isDark: boolean) {
    const element = document.querySelector('page-addmorephotos');
    if (element) {
      if (isDark) {
      ...
      setTimeout(() => {
        const retryElement = document.querySelector('page-addmorephotos');
        if (retryElement) {
          if (isDark) {
