...
private themeSub: Subscription;
  constructor(public socialSharing: SocialSharing,public fb: FirebaseService,private toastCtrl: ToastController,public commonService:CommonService,private renderer: Renderer2,private themeService: ThemeService,private storage: Storage,public events: Events,public navCtrl: NavController, public navParams: NavParams) {
  ...
  ionViewDidLoad() {
    ...
    this.loadTheme();
    this.themeSub = this.themeService.isDarkTheme$.subscribe((isDark) => this.applyTheme(isDark));
  ...
  async loadTheme() {
    let isDark = await this.storage.get('dashboardTheme');
  ...
  applyTheme(isDark: boolean) {
    ...
    const pageEl = document.querySelector('page-showeventsandnews');
    if (pageEl) {
      if (isDark) {
        this.renderer.removeClass(pageEl, 'light-theme');
      } else {
        this.renderer.addClass(pageEl, 'light-theme');
      }
    }
  }
