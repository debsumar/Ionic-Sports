...
@IonicPage()
@Component({
    selector: 'createonetoonesessioncoach-page',
    templateUrl: 'createonetoonesessioncoach.html'
})

export class CoachCreateOneToOneSession {
    themeType: number;
...
cancelSessionCreation(){
    ...
    onChangeActivityCategory() {
        this.activityCategoryList.forEach(element => {
            if (element.$key == this.selectActivityCategory) {
                ...
                console.log(this.activityCategoryObj);
            }
        });
    }
    ...
    createSession() {
        ...
        this.navCtrl.push("CoachAddMemberOneToOneSession", { ParentClubKey: this.sessionDetails.ParentClubKey, ClubKey: this.sessionDetails.ClubKey, CoachKey: this.sessionDetails.CoachKey, SessionKey: this.returnKey, SessionName: this.sessionDetails.SessionType, SessionDetials: this.sessionDetails });
