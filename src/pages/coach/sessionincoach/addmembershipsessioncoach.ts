...
@IonicPage()
@Component({
  selector: 'addmembershipsessioncoach-page',
  templateUrl: 'addmembershipsessioncoach.html'
})

export class CoachAddMembershipSession {
  themeType: number;
  venue: string = "s";
  term: string = "t1";
  ...
  addMember() {
    ...
    if (isExcute) {
      ...
      for (let j = 0; j < memberArray.length; j++) {
        ...
        this.fb.update(this.SessionKey, "/Member/" + this.parentClubKey + "/" + this.clubKey + "/" + memberKeys[j] + "/Session/", this.sessionDetailsObj);
